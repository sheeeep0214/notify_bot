        ...
        if doc and dc_id in doc.get("channels", {}):
            ...
            if platform == "ig":
                if IG_API_KEYS:
                    ...
                    await ctx.send("⚠️ 尚未設定任何 IG API Key。")
                    return
...
# ==========================================
# 💡 終極除錯指令：使用正確的 URL-encoded Form Data
# ==========================================
...
async def debug_api(ctx, platform: str, target_id: str):
    ...
    await ctx.send(f"🔍 正在對 `{target_id}` 透過 Form-Urlencoded 執行深度除錯...")
    
    posts_url = f"https://{IG_API_HOST}/get_ig_user_posts.php"
    form_data = urllib.parse.urlencode({"username": target_id})
...
# ==========================================
# 3. Instagram 監控輪詢 (使用 Form-Urlencoded 傳遞 username)
# ==========================================
@tasks.loop(hours=24) 
async def check_ig_updates():
    ...
    async with aiohttp.ClientSession() as session:
        for sub_doc in all_ig_subs:
            ...
            dc_channels = sub_doc.get("channels", {})
            form_data = urllib.parse.urlencode({"username": ig_username})
                ...
                try:
                    async with session.post(posts_url, headers=headers, data=form_data) as response:
                        ...
                        for item in reversed(items[:5]): 
                            ...
                            if await history_col.find_one({"ig_post_id": str(post_id)}):
                                continue
                                
                            await history_col.insert_one({"ig_post_id": str(post_id)})
                            
                            is_video = node.get("is_video", False)
                            media_type = str(node.get("type", "")).upper()
                            ...
                            post_url = node.get("url", f"https://www.instagram.com/p/{code}/" if code else f"https://www.instagram.com/{ig_username}/")
                            author_name = node.get("owner", {}).get("username", ig_username)
...
async def handle(request):
    return web.Response(text="Discord Bot is alive, using Form-Urlencoded for Instagram Scraper Stable API!")
